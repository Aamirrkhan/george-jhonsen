<?php
include './assets/componnets/header.php';
?>
<section class="organiz-banner auction-banner" style="background: url(assets/images/banner4.png);">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="banner-inner-dev">
                    <h1 class="banner-text">
                        January 2024 N Scale Enthusiast Auction
                        <span>
                        </span>
                    </h1>
                </div>
            </div>
        </div>
    </div>
</section>
<section>
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="auction-pdf-table">
                    <div class="overflow-table">
                        <div class="col-12">
                            <div class="table-parent">
                                <table
                                class="datatables-ajax table-responsive table-stripe table"
                                id="orders-table"
                                >
                                <thead>
                                    <tr class="tbl-head">
                                        <th>MFG</th>
                                        <th>MODEL #</th>
                                        <th>SCALE	</th>
                                        <th>ROAD NAME</th>
                                        <th>ROAD #</th>
                                        <th>DESCRIPTION</th>
                                        <th>ITEM #</th>
                                        <th>MINIMUM</th>
                                        <th>BUY IT NOW</th>
                                        <th class="after-before-none"></th>
                                    </tr>
                                </thead>
                                <tbody class="ord-pd-de">
                                    <tr class="even active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="odd active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="even active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="odd active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="even active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="odd active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="even active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="odd active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="even active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                    <tr class="odd active-tr">
                                        <td class="sorting_1">MTL</td>
                                        <td>2023</td>
                                        <td>N</td>
                                        <td>2023 NSE CUSTOM CAR - ARTIST PROOF</td>
                                        <td>NSE 2023 "AREA 52"</td>
                                        <td> ONLY 1 AVAILABLE</td>
                                        <td>1</td>
                                        <td>$300.00</td>
                                        <td>$300.00</td>
                                        <td class="order-view">https://www	<a href="">See In Trio</a></td>
                                    </tr>
                                </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>



<?php
include './assets/componnets/newsletter.php';
include './assets/componnets/footer.php';
?>
<script>
   $(function () {
   $("#orders-table").DataTable({
   processing: true,
   language: {
      paginate: {
         previous: '<i class="fa-solid fa-angle-left"></i>',
         next: '<i class="fa-solid fa-angle-right"></i>',
      },
   },
   columns: [
      {
         data: "MFG",
      },
      {
         data: "model #",
      },
      {
         data: "scale",
      },
      {
         data: "ROAD NAME",
      },
      {
         data: "ROAD #",
      },
      {
         data: "description",
      },
      {
         data: "ITEM #",
      },
      {
         data: "MINIMUM",
      },
      {
         data: "BUY IT NOW	",
      },
      {
         data: "",
      },
   ],
   });
   });
</script>